...
1. **瀏覽與登入**：Browse 分頁上方是來源選單，切換 Patreon、Google Docs、AO3、方格子、AsianFanfics、CXC、在水裡寫字。每個來源各自獨立的 WebView 與登入狀態，互不影響。在對應平台頁面用自己的帳號登入（Patreon 的 Google、Apple、Facebook 登入都在 app 內的彈出視窗完成）。
...
- 離線閱讀只限有本機內文的來源：AO3、Google Docs、在水裡寫字匯入時會把章節內文存在裝置上，之後不用網路也能讀。Patreon、方格子、AsianFanfics、CXC 每次都從網站載入，需要網路與登入，這是刻意的設計。
