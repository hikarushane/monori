...
3. **Library** 分頁選 collection，點章節開始讀。標成**追更中**的 collection 會在前景自動檢查新章節（Settings 可切換），新章節會有未讀標記，目錄裡也有紅點。工具列選單可以排序（標題／最近更新／最近閱讀）、搜尋標題或作者、依閱讀狀態篩選；下拉可以重新整理所有追更中的 collection。單一 collection 點 **•••** 可以設定閱讀狀態，或手動**檢查新章節**。
